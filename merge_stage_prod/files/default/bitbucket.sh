EXECUTE0=$(curl -X POST -H "Content-Type: application/json" -u suvadip30:'SUVAman$9614' https://bitbucket.org/api/2.0/repositories/suvadip30/test-patch/pullrequests -d '{ "title": "Merge some branches", "description": "stackoverflow example", "source": { "branch": { "name": "stage" }, "repository": { "full_name": "suvadip30/test-patch" } }, "destination": { "branch": { "name": "prod" } }, "reviewers": [ { "username": "some other user needed to review changes" } ], "close_source_branch": false }' | grep 'branch not found:\|There are no changes to be pulled' | wc -l)
