...
include_recipe 'git_drupal::default'

script '**********generate DB backup in tmp directory**********' do
  interpreter 'bash'
        code <<-EOH
...
script '**********enable google analytics MODULES*********' do
        interpreter 'bash'
        code <<-EOH
...
script '**********update drupal-core and other modules**********' do
        interpreter 'bash'
        code <<-EOH
        #{node['drupal_db']['change_directory']} #{node['drupal_db']['root_directory']}
...
script '**********disable google analytics MODULES**********' do
        interpreter 'bash'
        code <<-EOH
        #{node['drupal_db']['change_directory']} #{node['drupal_db']['root_directory']}
